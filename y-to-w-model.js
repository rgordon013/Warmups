...
export function createMannequin() {
  ...
  function pose(phase) {
    ...
    // One full phase travels from Y to W and back to Y. Smoothstep keeps both
    // endpoints controlled while the shoulder blades draw down and together.
    ...
    SIDES.forEach((side, index) => {
      ...
      const upperDirection = target.set(
        side.sign * THREE.MathUtils.lerp(0.48, 0.58, blend),
        THREE.MathUtils.lerp(0.88, -0.78, blend),
        THREE.MathUtils.lerp(-0.02, -0.16, blend)
      ).normalize().clone();
      const forearmDirection = new THREE.Vector3(
        side.sign * THREE.MathUtils.lerp(0.48, 0.62, blend),
        THREE.MathUtils.lerp(0.88, 0.74, blend),
        THREE.MathUtils.lerp(-0.02, 0.04, blend)
      ).normalize();

...
const ARM_PATH = Object.freeze({ outward: 0.30, circle: 0.954 });
const SIDES = Object.freeze([
  { suffix: 'L', sign: 1, phaseOffset: 0 },
  { suffix: 'R', sign: -1, phaseOffset: Math.PI }
]);
...
export function createMannequin() {
  ...
  const ready = new Promise((resolve, reject) => {
    new GLTFLoader().load('./assets/exercise-model.glb', async gltf => {
      ...
      const required = [...chains.flat(), spine, neck, head];
      ...
      rig = {
        character, byName, skinnedMeshes, bones, chains, spine, neck, rest,
        restSegmentLengths,
  ...
  function pose(phase) {
    ...
    // The shoulder girdle and torso move with the circles, as in the approved
    // demonstration, while the neck gently counters to keep the gaze steady.
    rig.spine.rotation.set(
      0.025 * Math.sin(phase * 2),
      0.14 * Math.sin(phase),
      0.035 * Math.cos(phase)
    );
    rig.neck.rotation.set(
      -rig.spine.rotation.x * 0.30,
      -rig.spine.rotation.y * 0.55,
      -rig.spine.rotation.z * 0.45
    );
    mesh.updateMatrixWorld(true);

    SIDES.forEach((side, index) => {
      const armPhase = phase + side.phaseOffset;
      target.set(
        side.sign * ARM_PATH.outward,
        -Math.cos(armPhase) * ARM_PATH.circle,
        Math.sin(armPhase) * ARM_PATH.circle
      ).normalize();
      aimChain(rig.chains[index], target);
    });
